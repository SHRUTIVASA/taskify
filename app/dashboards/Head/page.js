...
export default function HeadDashboard() {
  ...
  // UI & Selection States
  const [selectedUnitHeadUIDs, setSelectedUnitHeadUIDs] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
        try {
            const docSnap = await getDoc(doc(db, "heads", currentUser.uid));
            if (docSnap.exists()) setUserData(docSnap.data());
        } catch (e) { console.error(e); }
    };

    // Fetch Heads (For TaskRow context)
  ...
  // --- 3. Consolidated Task Reset Logic with Due Date Removal ---
  const checkAndResetTasks = useCallback(async (currentTasks) => {
    ...
    const updatedTasks = currentTasks.map(task => {
        ...
        if (shouldReset) {
            ...
            // Remove due date if it's in the past relative to the reset cycle
    ...
    if (needsUpdate) {
        try {
            await updateDoc(doc(db, "heads", currentUser.uid), { tasks: updatedTasks });
  ...
  useEffect(() => {
    ...
    // Listen to Head's own tasks
    const unsub = onSnapshot(doc(db, "heads", currentUser.uid), (docSnap) => {
        if (docSnap.exists()) {
            ...
            // Fetch Assigned Unit Heads
            if (data.assigned && data.assigned.length > 0) {
                fetchUnitHeads(data.assigned);
            } else {
                setUnitHeads([]);
            }
            setIsLoading(false);
  ...
  const fetchUnitHeads = async (unitHeadUIDs) => {
    try {
        const unitHeadsCollection = collection(db, "unitheads");
        const snapshot = await getDocs(unitHeadsCollection);
        const uhData = snapshot.docs
            .map(doc => ({ id: doc.id, ...doc.data() }))
            .filter(uh => unitHeadUIDs.includes(uh.uid));
        
        setUnitHeads(uhData);
    } catch (e) {
        console.error("Error fetching unit heads", e);
    }
  };
  ...
  // --- Actions ---
  const handleUnitHeadClick = async (unitHeadId) => {
      setIsLoading(true);
      setSelectedUnitHeadId(unitHeadId);
      
      try {
          const uh = unitHeads.find(u => u.uid === unitHeadId);
          if (uh && uh.assigned) {
              const tlPromises = uh.assigned.map(uid => getDoc(doc(db, "teamleaders", uid)));
              const snaps = await Promise.all(tlPromises);
              const tlData = snaps.filter(s => s.exists()).map(s => s.data());
      ...
      } catch (e) { setError("Failed to fetch team leaders"); }
  ...
  const handleTeamLeaderClick = async (teamLeaderId) => {
      ...
      try {
          ...
          if (tl && tl.assigned) {
              const supPromises = tl.assigned.map(uid => getDoc(doc(db, "supervisors", uid)));
              const snaps = await Promise.all(supPromises);
  ...
  const handleAddTask = async (e) => {
    ...
    if (selectedUnitHeadUIDs.length === 0) {
        showError("Select at least one unit head.");
        return;
    }
    ...
    try {
        ...
        const updates = await Promise.all(selectedUnitHeadUIDs.map(async (uid) => {
             const ref = doc(db, "unitheads", uid);
        ...
        if (validUpdates.length === 0) {
            setError("Task already assigned or invalid unit head.");
        ...
        // --- OPTIMISTIC UPDATE ---
        setUnitHeads(prev => prev.map(uh => {
            if (selectedUnitHeadUIDs.includes(uh.uid)) {
                return { ...uh, tasks: [...(uh.tasks || []), newTask] };
            }
            return uh;
        }));
  ...
  const handleChangeStatus = async (taskId, newStatus, fullTaskObject) => {
      const collections = ["heads", "unitheads", "teamleaders", "supervisors", "employees"];
      try {
      ...
      <AnimatePresence>
        {showProfile && (
           <ModalWrapper onClose={() => setShowProfile(false)}>
              <UserProfile onBack={() => setShowProfile(false)} currentUser={currentUser} userRole="head" />
           </ModalWrapper>
        )}
           ...
           <ModalWrapper onClose={() => setShowTaskForm(false)}>
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6">
                  ...
                  <form onSubmit={handleAddTask} className="space-y-4">
                      ...
                      <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">Assign to Unit Heads</label>
                          {unitHeads.length === 0 ? (
                             <p className="text-gray-500 text-sm">No unit heads available.</p>
                          ) : (
                              <div className="max-h-40 overflow-y-auto border border-gray-300 rounded p-2 space-y-1">
                                  {unitHeads.map(uh => (
                                      <div key={uh.uid} className="flex items-center">
                                          <input type="checkbox" checked={selectedUnitHeadUIDs.includes(uh.uid)} 
                                                 onChange={e => {
                                                     if(e.target.checked) setSelectedUnitHeadUIDs([...selectedUnitHeadUIDs, uh.uid]);
                                                     else setSelectedUnitHeadUIDs(selectedUnitHeadUIDs.filter(id => id !== uh.uid));
                                                 }}
                                                 className="mr-2 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                          />
                                          <span className="text-sm text-gray-700">{uh.name}</span>
                                      </div>
                                  ))}
                              </div>
                          )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
         <div className="mb-8">
            ...
            <div className="mt-2">
                ...
                <p className="text-sm text-gray-500 mt-1">Leading the organization</p>
            </div>
         </div>
         ...
         {showEmployeeList ? (
             ...
             <div className="mb-8 bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                <div className="px-6 py-5 border-b border-gray-200 flex justify-between items-center">
                    <div>
                         <h3 className="text-lg font-semibold text-gray-800">Team Leader Team</h3>
                         <p className="text-sm text-gray-500 mt-1">Team Leaders under selected unit head</p>
                    </div>
                    <button onClick={() => setShowTeamLeaderList(false)} className="flex items-center text-blue-600 hover:text-blue-800 transition-colors">
                        <FiArrowLeft className="mr-1"/> Back to Unit Heads
                    </button>
                </div>
                <div className="p-6">
             ...
             <div className="mb-8 bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                <div className="px-6 py-5 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-800">My Unit Heads</h3>
                    <p className="text-sm text-gray-500 mt-1">Unit Heads under your leadership</p>
                </div>
                <div className="p-6">
                    <UnitHeadList unitHeads={unitHeads} onUnitHeadClick={handleUnitHeadClick} onFilterTasks={() => {}} />
                </div>
             </div>
         )}
         ...
         <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-200">
                <h3 className="text-lg font-semibold text-gray-800">Head Task List</h3>
