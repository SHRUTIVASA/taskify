...
export default function TeamLeaderDashboard() {
  ...
  const [allHeadsData, setAllHeadsData] = useState([]); // For TaskRow N+1 fix
  ...
  useEffect(() => {
    ...
    // Fetch Unit Head (Boss)
    const fetchUnitHeadInfo = async () => {
      setIsBossLoading(true);
      setBossName(null); 
      try {
        const q = query(collection(db, "unitheads"), where("assigned", "array-contains", currentUser.uid));
    ...
    const fetchUserData = async () => {
        try {
            const docSnap = await getDoc(doc(db, "teamleaders", currentUser.uid));
...
const checkAndResetTasks = useCallback(async (currentTasks) => {
    ...
    if (needsUpdate) {
        try {
            await updateDoc(doc(db, "teamleaders", currentUser.uid), { tasks: updatedTasks });
            console.log("Tasks reset and old due dates removed successfully");
        } catch (e) { console.error(e); }
    }
  }, [currentUser]);
  ...
  useEffect(() => {
    ...
    // Listen to Team Leader's own tasks
    const unsub = onSnapshot(doc(db, "teamleaders", currentUser.uid), (docSnap) => {
        if (docSnap.exists()) {
            ...
            // Also fetch supervisors if assigned array exists
            if (data.assigned && data.assigned.length > 0) {
                 fetchSupervisors(data.assigned);
            } else {
                 setSupervisors([]);
            }
            setIsLoading(false);
  ...
  const fetchSupervisors = async (supervisorUIDs) => {
      try {
          const supervisorsCollection = collection(db, "supervisors");
          const snapshot = await getDocs(supervisorsCollection);
          const supervisorsData = snapshot.docs
            .map(doc => ({ id: doc.id, ...doc.data() }))
            .filter(sup => supervisorUIDs.includes(sup.uid));
  ...
  const handleSupervisorClick = async (supervisorId) => {
    ...
    try {
      ...
      if (supervisor && supervisor.assigned) {
          const empPromises = supervisor.assigned.map(uid => getDoc(doc(db, "employees", uid)));
          const empSnaps = await Promise.all(empPromises);
          const empData = empSnaps.filter(s => s.exists()).map(s => s.data());
  ...
  const handleAddTask = async (e) => {
    ...
    if (selectedsupervisorUIDs.length === 0) {
        showError("Select at least one supervisor.");
        return;
    }
    ...
    try {
        ...
        const updates = await Promise.all(selectedsupervisorUIDs.map(async (uid) => {
             const ref = doc(db, "supervisors", uid);
        ...
        if (validUpdates.length === 0) {
            setError("Task already assigned or invalid supervisor.");
        ...
        // --- OPTIMISTIC UPDATE ---
        // Update local supervisors state immediately so the list updates instantly
        setSupervisors(prev => prev.map(sup => {
            if (selectedsupervisorUIDs.includes(sup.uid)) {
                return { ...sup, tasks: [...(sup.tasks || []), newTask] };
            }
            return sup;
        }));
  ...
  // Handle Status Change for TL's own tasks
  const handleChangeStatus = async (taskId, newStatus, fullTaskObject) => {
    const collections = ["teamleaders", "supervisors", "employees", "unitheads", "heads"];
    try {
      ...
      <AnimatePresence>
        {showProfile && (
           <ModalWrapper onClose={() => setShowProfile(false)}>
              <UserProfile onBack={() => setShowProfile(false)} currentUser={currentUser} userRole="teamleader" />
           </ModalWrapper>
        )}
           ...
           <ModalWrapper onClose={() => setShowTaskForm(false)}>
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6">
                  ...
                  <form onSubmit={handleAddTask} className="space-y-4">
                      ...
                      <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">Assign to Supervisors</label>
                          {supervisors.length === 0 ? (
                             <p className="text-gray-500 text-sm">No supervisors available.</p>
                              ...
                              <div className="max-h-40 overflow-y-auto border border-gray-300 rounded p-2 space-y-1">
                                  {supervisors.map(sup => (
                                      <div key={sup.uid} className="flex items-center">
                                          <input type="checkbox" checked={selectedsupervisorUIDs.includes(sup.uid)} 
                                                 onChange={e => {
                                                     if(e.target.checked) setSelectedsupervisorUIDs([...selectedsupervisorUIDs, sup.uid]);
                                                     else setSelectedsupervisorUIDs(selectedsupervisorUIDs.filter(id => id !== sup.uid));
                                                 }}
         ...
         <div className="mb-8">
            ...
            <div className="mt-2">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    {isBossLoading ? "Loading info..." : bossName ? `Reporting to: ${bossName} (Unit Head)` : "No Unit Head assigned"}
                </p>
            </div>
         </div>
         ...
         {showEmployeeList ? (
             ...
             <div className="mb-8 bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                <div className="px-6 py-5 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-800">My Supervisors</h3>
                    <p className="text-sm text-gray-500 mt-1">Supervisors under your leadership</p>
                </div>
                <div className="p-6">
                    <SupervisorList supervisors={supervisors} onSupervisorClick={handleSupervisorClick} onFilterTasks={() => {}} />
                </div>
             </div>
         )}
